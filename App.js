...
// Coordonnées approximatives (niveau ville) utilisées pour générer les vignettes de
// carte des terrains publics — précision suffisante pour un aperçu visuel façon Google Maps.
const CITY_COORDS = {
...
// Convertit lat/lon en coordonnées de tuile OpenStreetMap (format XYZ standard)
// + position en pixels du point exact à l'intérieur de cette tuile (256x256).
function latLonToTile(lat, lon, zoom) {
  const latRad = (lat * Math.PI) / 180;
  const n = Math.pow(2, zoom);
  const worldX = ((lon + 180) / 360) * n * 256;
  const worldY = ((1 - Math.log(Math.tan(latRad) + 1 / Math.cos(latRad)) / Math.PI) / 2) * n * 256;
  const xtile = Math.floor(worldX / 256);
  const ytile = Math.floor(worldY / 256);
  return { xtile, ytile, px: worldX - xtile * 256, py: worldY - ytile * 256 };
}
...
const FRENCH_STADIUMS = [
  ...
  { name: 'Stade Léo-Lagrange', city: 'Besançon' },
  ...
  { name: 'Stade Pierre-de-Coubertin', city: 'Cannes' },
...
// Vignette "carte" façon Google Maps pour un terrain : tuile OpenStreetMap centrée
// sur la ville + épingle positionnée précisément dessus. Gratuit, sans clé API.
function VenueMapThumb({ city, size = 90 }) {
  const coords = CITY_COORDS[city];
  if (!coords) {
    return (
      <View style={[s.venueMapThumb, { width: size, height: size, alignItems: 'center', justifyContent: 'center' }]}>
        <Ionicons name="map-outline" size={22} color={C.light} />
      </View>
    );
  }
  const zoom = 13;
  const { xtile, ytile, px, py } = latLonToTile(coords[0], coords[1], zoom);
  const tileUrl = `https://tile.openstreetmap.org/${zoom}/${xtile}/${ytile}.png`;
  return (
    <View style={[s.venueMapThumb, { width: size, height: size }]}>
      <Image source={{ uri: tileUrl }} style={{ width: 256, height: 256, position: 'absolute', left: -px + size / 2, top: -py + size / 2 }} />
      <View style={{ position: 'absolute', left: size / 2 - 10, top: size / 2 - 20 }}>
        <Ionicons name="location" size={20} color={C.danger} />
      </View>
    </View>
  );
}
...
function VenuesScreen({ go, goBack }) {
  ...
  // Liste complète filtrée par la recherche (terrains publics uniquement)
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={s.venueCard}>
            <View style={{ flexDirection: 'row' }}>
              <VenueMapThumb city={item.city} size={72} />
              <View style={{ flex: 1, marginLeft: 12 }}>
            ...
            <Text style={{ color: C.light, fontSize: 10, marginTop: 2 }}>© OpenStreetMap contributors</Text>
...
function BarcodeScanScreen({ go, goBack, params }) {
  ...
  const searchResults = showSearch ? searchFood(searchQuery) : [];
  ...
  const handleSelectFromSearch = (food) => {
    const searchProduct = {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0B0B14' }}>
      ...
      {/* Toggle scanner / recherche / historique */}
      <View style={{ flexDirection: 'row', paddingHorizontal: 16, marginBottom: 12, gap: 8 }}>
        <TouchableOpacity
          onPress={() => { setShowHistory(false); setShowSearch(false); }}
          style={{ flex: 1, alignItems: 'center', paddingVertical: 10, borderRadius: 12, backgroundColor: (!showHistory && !showSearch) ? C.primary : 'rgba(255,255,255,0.06)' }}
        >
          <Text style={{ fontWeight: '800', fontSize: 12, color: (!showHistory && !showSearch) ? '#fff' : 'rgba(255,255,255,0.5)' }}>📷 SCANNER</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => { setShowSearch(true); setShowHistory(false); }}
          style={{ flex: 1, alignItems: 'center', paddingVertical: 10, borderRadius: 12, backgroundColor: showSearch ? C.primary : 'rgba(255,255,255,0.06)' }}
        >
          <Text style={{ fontWeight: '800', fontSize: 12, color: showSearch ? '#fff' : 'rgba(255,255,255,0.5)' }}>
            🔍 RECHERCHE
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => { setShowHistory(true); setShowSearch(false); }}
      ...
      {showSearch ? (
        /* Recherche manuelle dans la base de produits (sans scan caméra) */
        <ScrollView contentContainerStyle={{ padding: 16, paddingTop: 0 }} keyboardShouldPersistTaps="handled">
          <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 14, paddingHorizontal: 14, marginBottom: 12 }}>
            <Ionicons name="search" size={18} color="rgba(255,255,255,0.4)" />
            <TextInput
              style={{ flex: 1, color: '#fff', paddingVertical: 12, paddingHorizontal: 10, fontSize: 14 }}
              placeholder="Chercher un produit (ex: Nutella, pomme, yaourt...)"
          ...
          {searchQuery.trim().length === 0 ? (
            <View style={{ alignItems: 'center', paddingVertical: 40 }}>
              <Ionicons name="fast-food-outline" size={40} color="rgba(255,255,255,0.3)" />
              <Text style={{ color: 'rgba(255,255,255,0.5)', marginTop: 8, textAlign: 'center' }}>
                Tape le nom d'un produit alimentaire pour le retrouver sans scanner son code-barres.
              </Text>
            </View>
          ) : searchResults.length === 0 ? (
            <View style={{ alignItems: 'center', paddingVertical: 40 }}>
              <Ionicons name="sad-outline" size={40} color="rgba(255,255,255,0.3)" />
              <Text style={{ color: 'rgba(255,255,255,0.5)', marginTop: 8 }}>Aucun produit trouvé</Text>
            </View>
          ) : (
            searchResults.map((food) => (
              <TouchableOpacity
                key={food.dish}
                onPress={() => handleSelectFromSearch(food)}
                style={{ backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 16, padding: 14, marginBottom: 10 }}
              >
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  <View style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.08)', alignItems: 'center', justifyContent: 'center' }}>
                    <Text style={{ fontSize: 20 }}>{CATEGORY_EMOJIS[food.category] || '🍽️'}</Text>
                  </View>
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <Text style={{ fontWeight: '800', fontSize: 14, color: '#fff' }} numberOfLines={1}>{food.dish}</Text>
                    <Text style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11, marginTop: 2 }}>
                      {food.kcal} kcal · {food.portion_g}g · P {food.protein_g}g · G {food.carbs_g}g · L {food.fat_g}g
                    </Text>
                  </View>
            ...
            <View style={{ margin: 16, padding: 20, backgroundColor: C.card, borderRadius: 20, borderWidth: 1, borderColor: C.border }}>
              ...
              {product.fromSearch ? (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#EEF0FF', alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 999, marginBottom: 12 }}>
                  ...
                  <Text style={{ color: C.primary, fontWeight: '800', fontSize: 11 }}>AJOUTÉ VIA RECHERCHE</Text>
                </View>
              ) : product.fromCache ? (
...
export default function App() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}>
      {tabBackground ? (
        <Grad colors={tabBackground} style={{ flex: 1 }}>
...
const s = StyleSheet.create({
  ...
  venueMapThumb: { borderRadius: 14, overflow: 'hidden', backgroundColor: C.chip },

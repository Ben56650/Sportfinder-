...
function tipForDiet(name) {
  ...
  if (key.includes('intermittent') || key.includes('jeûne') || key.includes('jeune')) {
    return 'En jeûne intermittent, concentrez vos apports caloriques sur votre fenêtre de repas et priorisez les protéines pour préserver votre masse musculaire.';
  }
  if (!key) return null;
...
function NutritionGoalScreen({ goBack }) {
  ...
  const save = async () => {
    const goal = { mealsPerDay, targetKcal: parseInt(targetKcal, 10) || 2000, dietName };
    try {

...
// ---------- STORIES D'ACTIVITÉ ----------
const STORIES = [
  { id: 's1', userId: 'f1', name: 'Alice Runner', emoji: '🏃', type: 'run', data: '5.2 km', time: '2h', viewed: false, color: '#22C55E' },
  { id: 's2', userId: 'f2', name: 'Marc', emoji: '🏀', type: 'basket', data: '12 pts', time: '4h', viewed: false, color: '#F59E0B' },
  { id: 's3', userId: 'f3', name: 'Sofia', emoji: '🧘', type: 'yoga', data: '45 min', time: '6h', viewed: true, color: '#EC4899' },
  { id: 's4', userId: 'f4', name: 'Cal Tester', emoji: '⚡', type: 'hiit', data: '320 kcal', time: '8h', viewed: false, color: '#EF4444' },
...
async function getSubscriptionStatus() {
  try {
    ...
    if (!raw) return { isPremium: false, eventsCreated: 0, trialUsed: 0 };
    return JSON.parse(raw);
  } catch (e) {
    return { isPremium: false, eventsCreated: 0, trialUsed: 0 };
  }
}
...
async function canCreateEvent() {
  const sub = await getSubscriptionStatus();
  if (sub.isPremium) return { allowed: true, remaining: -1 };
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 48 }} keyboardShouldPersistTaps="handled">
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: subStatus.isPremium ? 'rgba(34,197,94,0.1)' : C.chip, paddingHorizontal: 14, paddingVertical: 12, borderRadius: 12, marginBottom: 16, borderWidth: 1, borderColor: subStatus.isPremium ? '#22C55E' : C.border }}>
          ...
          <Text style={{ color: subStatus.isPremium ? '#22C55E' : C.primary, fontWeight: '800', fontSize: 12, flex: 1 }}>
            {subStatus.isPremium ? '💎 PREMIUM — Événements illimités' : `GRATUIT — ${subStatus.remaining} création${subStatus.remaining > 1 ? 's' : ''} restante${subStatus.remaining > 1 ? 's' : ''}`}
          </Text>
          {!subStatus.isPremium ? (

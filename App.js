...
const PUSHUP_SENSITIVITIES = [
  { key: 'low', label: 'Faible', threshold: 0.15 },
  { key: 'medium', label: 'Moyenne', threshold: 0.28 },
  { key: 'high', label: 'Élevée', threshold: 0.45 },
];
const PUSHUP_MIN_REP_INTERVAL_MS = 500; // anti-rebond : évite de compter 2x le même mouvement
...
function PushupScreen({ goBack }) {
  ...
  const start = async () => {
    ...
    subRef.current = Accelerometer.addListener(({ x, y, z }) => {
      ...
      const deviation = magnitude - 1; // écart par rapport à la gravité au repos (~1g)
      setLiveAmplitude(deviation);

      const threshold = PUSHUP_SENSITIVITIES.find((s2) => s2.key === sensitivityRef.current).threshold;
      const now = Date.now();

      if (cycleStateRef.current === 'up' && deviation < -threshold) {
        cycleStateRef.current = 'down';
      } else if (cycleStateRef.current === 'down' && deviation > threshold) {
        cycleStateRef.current = 'up';
  ...
  const currentThreshold = PUSHUP_SENSITIVITIES.find((s2) => s2.key === sensitivity).threshold;
  const amplitudeRatio = Math.min(1, Math.abs(liveAmplitude) / (currentThreshold * 1.5));
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0B0B14' }}>
      <ScrollView>
        ...
        <View style={s.trackInfo}>
          <Ionicons name="information-circle" size={18} color={C.primary} />
          <Text style={s.trackInfoText}>
            Cale le téléphone contre ton corps (brassard, poche de hanche ou ceinture ajustée) pour qu’il bouge AVEC toi — posé au sol, il ne détecte rien puisque c’est ton corps qui bouge, pas lui.
          </Text>
        </View>
        ...
        <View style={s.modesRow}>
          {PUSHUP_SENSITIVITIES.map((s2) => (
            <TouchableOpacity
              key={s2.key}
              onPress={() => setSensitivity(s2.key)}
              style={[s.modeBtn, sensitivity === s2.key && s.modeBtnOn]}
            >
              <Text style={[s.modeText, sensitivity === s2.key && s.modeTextOn]}>{s2.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
        <Text style={s.pushupHint}>Si rien ne se compte : baisse la sensibilité. Si ça compte trop vite : monte-la.</Text>

        {tracking ? (
          <View style={s.amplitudeBox}>
            <Text style={s.amplitudeLabel}>SIGNAL CAPTEUR EN DIRECT</Text>
            <View style={s.amplitudeTrack}>
              <View style={[s.amplitudeFill, { width: `${amplitudeRatio * 100}%` }]} />
            </View>
            <Text style={s.amplitudeSub}>
              Si la barre ne bouge jamais quand tu remues le téléphone, le capteur ne transmet rien — redémarre l’app.
            </Text>
          </View>
        ) : null}

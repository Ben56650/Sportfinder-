...
function MatchScreen({ go, goBack }) {
  ...
  const swipe = async (liked) => {
    ...
    if (liked && top) {
      ...
      setLastMatch(top);
      await incrementStat('matchesSwiped');
    }
  ...
  if (hasPermission === false) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: '#0B0B14', justifyContent: 'center', alignItems: 'center', padding: 24 }}>
        ...
        <TouchableOpacity onPress={() => BarCodeScanner.requestPermissionsAsync()} style={{ marginTop: 24 }}>
          <Grad colors={C.gradOrangePink} style={[s.startBtn, { paddingHorizontal: 32 }]}>
            <Text style={s.startText}>📷 AUTORISER</Text>
          </Grad>
        </TouchableOpacity>
        ...
        <>
          ...
          <View style={{ flex: 1, marginHorizontal: 16, borderRadius: 24, overflow: 'hidden', borderWidth: 2, borderColor: C.primary }}>
            ...
            <TouchableOpacity
              ...
              style={{ position: 'absolute', top: 16, right: 16, width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center' }}
            >

  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      <TouchableOpacity onPress={() => go('match')}>
        <Banner icon="🤝" eyebrow="RENCONTRES SPORTIVES" title="Trouver une rencontre" subtitle={`Événements à venir autour de toi · jusqu'à ${maxDistance} km`} />
      </TouchableOpacity>
...
function NewEventScreen({ go, goBack, params }) {
  ...
  const submit = async () => {
    ...
    const levelLabel = { debutant: 'Débutant', intermediate: 'Intermédiaire', confirme: 'Confirmé', expert: 'Expert' }[level];
    alert(`Événement "${title}" créé !\nNiveau: ${levelLabel}${isCoachMode ? '\n🥋 Séance coach à ' + coachPrice + '€' : ''}`);
    go('events');
  };
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 48 }} keyboardShouldPersistTaps="handled">
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 8 }}>
          {[
            { key: 'debutant', label: 'Débutant', emoji: '🌱', desc: 'Je découvre ce sport' },
            { key: 'intermediate', label: 'Intermédiaire', emoji: '⭐', desc: 'Niveau régulier' },
            { key: 'confirme', label: 'Confirmé', emoji: '🔥', desc: 'Bonne maîtrise technique' },
            { key: 'expert', label: 'Expert', emoji: '💎', desc: 'Compétition / Haut niveau' },
          ].map((lvl) => (
            <TouchableOpacity
              key={lvl.key}
              ...
              style={{
                ...
                paddingVertical: 10,
                borderRadius: 12,
                backgroundColor: level === lvl.key ? C.primary : C.card,
                borderWidth: 1.5,
                borderColor: level === lvl.key ? C.primary : C.border,
                marginRight: 8,
                marginBottom: 8,
              }}
            >
              ...
              <View>
                <Text style={{ fontWeight: '800', fontSize: 12, color: level === lvl.key ? '#fff' : C.dark }}>{lvl.label.toUpperCase()}</Text>
                <Text style={{ fontSize: 10, color: level === lvl.key ? 'rgba(255,255,255,0.8)' : C.gray }}>{lvl.desc}</Text>
              </View>
            </TouchableOpacity>
          ))}
        </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        >
          ...
          <Text style={{ color: '#fff', fontWeight: '800', fontSize: 14 }}>
            {showAddForm ? 'ANNULER' : '+ AJOUTER UN REPAS'}
          </Text>
        </TouchableOpacity>

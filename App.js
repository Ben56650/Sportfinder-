...
function SwipeCard({ event, isTop, onSwipe }) {
  ...
  return (
    <Animated.View {...(isTop ? responder.panHandlers : {})} style={[s.swipeCard, isTop && { transform: [{ translateX: pan.x }, { translateY: pan.y }, { rotate }] }]}>
      <Grad colors={C.gradPurple} style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
        <Text style={{ fontSize: 90 }}>{event.emoji}</Text>
      </Grad>
      <View style={s.swipeInfo}>
        <Text style={s.swipeName}>{event.title}</Text>
        <Text style={s.swipeMeta}>📍 {event.loc}</Text>
        <Text style={s.swipeMeta}>🕐 {event.date} · Hôte : {event.host}</Text>
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
          <Text style={s.swipeChip}>
            {event.emoji} {(ALL_SPORTS.find((sp) => sp.key === event.sport)?.label || event.sport).toUpperCase()}
          </Text>
          <Text style={s.swipeChip}>
            👥 {event.p}/{event.max}{full ? ' · COMPLET' : ''}
          </Text>
        </View>
      </View>
    </Animated.View>
...
function MatchScreen({ go, goBack }) {
  ...
  const loadDeck = async () => {
    const ids = await getJoinedEventIds();
    const joinedMap = Object.fromEntries(ids.map((id) => [id, true]));
    setJoinedIds(joinedMap);
    setEvents(EVENTS.filter((e) => !joinedMap[e.id]));
  };
  ...
  const swipe = async (liked) => {
    const top = events[0];
    if (liked && top) {
      await setEventJoined(top.id, true);
      setJoinedIds((prev) => ({ ...prev, [top.id]: true }));
      setLastJoined(top);
    }
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <Text style={{ paddingHorizontal: 16, color: C.gray, marginBottom: 12 }}>
        Glisse à droite pour t’inscrire à l’événement, à gauche pour passer
      </Text>

      {lastJoined ? (
        <View style={s.matchJoinedBanner}>
          <Ionicons name="checkmark-circle" size={16} color="#15803D" />
          <Text style={s.matchJoinedText}>Inscrit·e à « {lastJoined.title} » ✓</Text>
        </View>
      ) : null}
      ...
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
        {events.length === 0 ? (
          <EmptyState
            ...
            title="Plus d’événements à découvrir"
            subtitle="Reviens plus tard, ou crée ta propre rencontre !"
            cta="RAFRAÎCHIR"
            onCta={loadDeck}
          />
        ) : (
          events
            .slice(0, 2)
            .reverse()
            .map((ev, idx, arr) => <SwipeCard key={ev.id} event={ev} isTop={idx === arr.length - 1} onSwipe={swipe} />)
        )}
      </View>
      {events.length > 0 ? (
        <View style={s.swipeActions}>
          ...
          <TouchableOpacity style={[s.roundBtn, { backgroundColor: C.chip }]} onPress={() => swipe(true)}>
...
// Détection basée sur l'amplitude totale du mouvement (magnitude - gravité au repos ≈ 1g),
// plus robuste qu'un seul axe fixe car indépendante de l'orientation exacte du téléphone.
...
function PushupScreen({ goBack }) {
  ...
  const [threshold, setThreshold] = useState(0.06); // seuil très bas par défaut : signal de vibration faible
  ...
  // Téléphone posé au sol = il ne bouge pas lui-même. La seule chose détectable est le
  // "choc"/la vibration transmise par le sol à chaque appui des mains (surtout en bas de
  // la pompe). On mesure donc le "jerk" (variation brutale d'accélération d'une lecture à
  // l'autre) plutôt qu'un mouvement soutenu — plus proche d'un détecteur de coup que d'un
  // détecteur de cycle. Signal plus faible et plus sensible aux vibrations parasites (pas,
  // sol qui bouge) qu'un téléphone porté sur le corps.
  const listenSensor = () => {
  ...
  const calibrate = async () => {
    ...
    setTimeout(() => {
      ...
      if (peak < 0.015) {
        setCalibrationResult('none'); // aucune vibration détectée du tout
        return;
      }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0B0B14' }}>
      <ScrollView>
        ...
        <View style={s.trackInfo}>
          <Ionicons name="information-circle" size={18} color={C.primary} />
          <Text style={s.trackInfoText}>
            Pose le téléphone au sol, juste sous tes mains ou entre elles, sur un sol dur (pas de tapis épais). Le capteur détecte la vibration transmise au sol à chaque appui, pas le téléphone lui-même — signal plus faible qu’un capteur porté sur le corps.
          </Text>
        </View>
        ...
        <View style={s.thresholdBox}>
          ...
          <Text style={s.pushupHint}>Rien ne se compte : baisse (−). Ça compte trop vite (pas, bruits) : monte (+).</Text>
          ...
          {calibrationResult === 'none' ? (
            <Text style={s.calibrateResultBad}>Aucune vibration détectée pendant la calibration — rapproche le téléphone de tes mains, essaie un sol plus dur, ou repasse en mode brassard.</Text>
          ) : calibrationResult ? (
            <Text style={s.calibrateResultGood}>Pic mesuré : {calibrationResult} → seuil ajusté automatiquement à {threshold.toFixed(2)}.</Text>
          ) : null}
        </View>
